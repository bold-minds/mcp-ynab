...
A **read-only** Model Context Protocol server for the [YNAB](https://www.ynab.com) budgeting API. Lets an LLM (Claude Desktop, Cursor, pocketcrew, etc.) inspect your plans, accounts, categories, transactions, and monthly summaries — without the ability to modify anything.
...
Add to your team's `mcp/servers.yaml`:
...
servers:
  ynab:
    command: mcp-ynab
    env:
      YNAB_API_TOKEN_FILE: /run/secrets/ynab_token
```
